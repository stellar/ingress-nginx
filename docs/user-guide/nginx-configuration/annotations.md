...
This annotation is of the form `nginx.ingress.kubernetes.io/custom-response-headers: "<headers>"` to specify a custom response header. This annotation uses `more_set_headers` nginx directive. To specify multiple headers, you can use `||` to seperate multiple response headers:
